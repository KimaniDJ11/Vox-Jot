...
- Notary credentials must be configured: a stored `voxjot-notary` notarytool keychain profile (set up once with `bun run mac:setup-notary`), or `APPLE_ID` + `APPLE_PASSWORD` + `APPLE_TEAM_ID`, or `APPLE_API_KEY` + `APPLE_API_ISSUER` + `APPLE_API_KEY_PATH`. The command preflights these and fails before building if none are valid.
