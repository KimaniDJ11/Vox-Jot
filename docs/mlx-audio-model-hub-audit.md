...
| -------- | ------------------------- | ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P0       | LongCat-AudioDiT 1B       | TTS                    | [`mlx-community/LongCat-AudioDiT-1B-4bit`](https://hf.co/mlx-community/LongCat-AudioDiT-1B-4bit), plus bf16/5/6/8-bit variants                                             | Strong EN/ZH diffusion TTS with zero-shot voice cloning. Good missing voice-cloning option.                                        | Public. MIT upstream per MLX-Audio README. Needs MLX-Audio 0.4.3 validation. Use 4-bit as default, bf16 as quality option. | Add `mlx_longcat_audiodit` provider, model entries, voice-clone controls: `ref_audio`, `ref_text`, `guidance_method`, `cfg_strength`, `steps`. Add ProviderIcon monogram/mark for LongCat or Meituan. |
...
| P1       | Higgs Audio v2 3B         | TTS                    | [`mlx-community/higgs-audio-v2-3B-mlx-q6`](https://hf.co/mlx-community/higgs-audio-v2-3B-mlx-q6), [`q8`](https://hf.co/mlx-community/higgs-audio-v2-3B-mlx-q8)             | Real-time voice cloning across EN/ZH/KO/DE/ES. Strong Listen feature candidate.                                                    | Public. Apache-2.0. Needs Higgs tokenizer assets; verify `mlx-community/higgs-audio-v2-tokenizer`.                         | Add `mlx_higgs_audio` provider. Add tokenizer dependency validation and voice-cloning controls. Use q6 default, q8 quality option.                                                                    |
...
| MOSS-TTS / MOSS-TTS Local Transformer | TTS                       | [`OpenMOSS-Team/MOSS-TTS`](https://hf.co/OpenMOSS-Team/MOSS-TTS), [`OpenMOSS-Team/MOSS-TTS-Local-Transformer`](https://hf.co/OpenMOSS-Team/MOSS-TTS-Local-Transformer) | Labs                                     | Public Apache-2.0, 8.5B / 3.1B parameter class. Upstream code says streaming is not implemented yet. Needs MOSS audio tokenizer.                                                                                               |
...
| OmniVoice                             | TTS                       | [`k2-fsa/OmniVoice`](https://hf.co/k2-fsa/OmniVoice)                                                                                                                   | Prototype                                | Public Apache-2.0, 646+ language tags and voice cloning. MLX-Audio README shows a more manual load path with Higgs tokenizer and `torchaudio` CPU encode for cloning, so it likely needs a custom bridge before one-click use. |
...
| ------------- | ----------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------- |
| Qwen3 TTS     | `Qwen3-TTS-12Hz-0.6B-CustomVoice-bf16`, `1.7B-CustomVoice-bf16`, `1.7B-Base-8bit`, `CustomVoice-6bit` | Gives clearer hub choice between Base, CustomVoice, and VoiceDesign.                                                           | Same Qwen provider/icon. Needs correct generate method mapping, not just `model.generate()`. |
...
| VoxCPM2       | bf16 and 8-bit                                                                                        | Quality/performance choice beyond current 4-bit.                                                                               | Same provider; high memory for bf16.                                                         |
...
5. Validation:
   ...
   - For TTS, use `scripts/run-tts-model-eval.py` style smoke tests for generation success, output sample rate, and first-audio latency.
