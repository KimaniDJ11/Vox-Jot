...
Vox Jot also has non-MLX or file-analysis coverage for several adjacent models: native/ONNX Whisper, Parakeet, Moonshine, SenseVoice, GigaAM, plus speech-analysis adapters for Granite, Cohere Transcribe, pyannote, NeMo Sortformer, MLX Sortformer, Reverb, WhisperX, and Polyvoice. Heavy Python speech-analysis adapters route through the public managed runtime group at `IrieDinamik/vox-jot-speech-analysis-runtime`.
...
| Llama OuteTTS 1B 4-bit                | TTS                       | [`mlx-community/Llama-OuteTTS-1.0-1B-4bit`](https://hf.co/mlx-community/Llama-OuteTTS-1.0-1B-4bit)                                                                                                       | Tested                                   | Public MLX checkpoint. Replaces the Qwen3 OuteTTS 0.6B variants, which currently load but do not emit decodable audio tokens under mlx-audio 0.4.3. Full hard-suite score: 86.7, 5/5 cases, average WER 0.3023.                                                                         |
| MOSS-TTS / MOSS-TTS Local Transformer | TTS                       | [`mlx-community/MOSS-TTS-8B-8bit`](https://hf.co/mlx-community/MOSS-TTS-8B-8bit), [`mlx-community/MOSS-TTS-Local-Transformer-MLX-8bit`](https://hf.co/mlx-community/MOSS-TTS-Local-Transformer-MLX-8bit) | Removed from app catalog                 | Public Apache-2.0 MLX 8-bit conversions exist, but both failed the full Vox Jot hard suite. Local Transformer requires a different `mlx-speech` runtime and fails in mlx-audio with shape errors; 8B needs an unreleased MOSS-TTS loader not present in PyPI `mlx-audio==0.4.3`.        |
...
| Cohere Transcribe 03-2026             | STT                       | [`CohereLabs/cohere-transcribe-03-2026`](https://hf.co/CohereLabs/cohere-transcribe-03-2026)                                                                                                             | User-facing speech analysis              | Apache-2.0 but gated. Requires accepted HF terms and saved read token, then runs through the managed speech-analysis runtime.                                                                                                                                                           |
...
| Smart Turn v3                         | Turn detection            | [`mlx-community/smart-turn-v3`](https://hf.co/mlx-community/smart-turn-v3)                                                                                                                               | Blocked for catalog                      | BSD-2-Clause. Endpoint/turn-completion detector, not STT/TTS. Keep out of model selection for dictation.                                                                                                                                                                                |
...
5. Validation:
   ...
   - For file-analysis models, run the full committed sample set with `bun run eval:file-asr`; single-sample runs are smoke checks only and must not drive Testing view ranks.
