...
| P1       | Higgs Audio v3                 | TTS / cloning           | `bosonai/higgs-audio-v3-tts-4b`                                              | Newer 4B conversational TTS with broader language coverage than v2.              | Blocked as of 2026-06-07. The upstream repo is public and `mlx-audio` contains a `higgs_audio_v3` module, but the license is research/non-commercial only and Vox Jot's current loader fails the official `higgs_multimodal_qwen3` config before weights load. Do not expose until the loader path is fixed and full app-path TTS/style/clone suites pass. If later exposed for users with their own Boson commercial license, use the `commercial_license_required` BYO-license gate and do not mirror weights unless Vox Jot has distribution rights. |
...
- Higgs Audio v3: keep out of the user-facing TTS catalog until the runtime path works. `bosonai/higgs-audio-v3-tts-4b` is not open-source/permissive for default Vox Jot distribution; production, hosted API, revenue-generating, or end-user app use requires a separate Boson commercial license. The app-managed `mlx-audio` venv includes a `higgs_audio_v3` module, but a config-only probe against the official repo fails because `load_model` tries to import `higgs_multimodal_qwen3` instead of remapping it to `higgs_audio_v3`. With `model_type` patched to `higgs_audio_v3`, the loader reaches weight loading, but full 9.3 GB synthesis was intentionally not run because the license and app-path blockers already prevent exposure. If this becomes user-facing for users who bring their own Boson license, add it only behind `LicenseAcknowledgementGate.kind = "commercial_license_required"` and keep weights upstream/user-initiated unless Vox Jot has redistribution rights.
