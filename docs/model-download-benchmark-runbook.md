...
- July 5, 2026 security/runtime update: GitHub flagged `transformers==4.53.3` in the managed speech-analysis runtime as vulnerable. The runtime now resolves with `transformers==5.3.0` and `huggingface-hub==1.4.1`; NeMo Sortformer is excluded because the latest tested `nemo_toolkit[asr]==2.7.3` still requires `transformers~=4.57.0`, below the patched floor. Do not re-add NeMo to the visible catalog or shared requirements until NeMo resolves with `transformers>=5.3.0`. WhisperX remains pinned at `whisperx==3.4.2`.
