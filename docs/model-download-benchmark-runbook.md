...
- DiariZen is removed from the user-facing app catalog and Testing view. Granite, Cohere Transcribe, PyAnnote, Reverb, and Polyvoice stay in the user-facing speech-analysis hub and route through the managed speech-analysis runtime group at `IrieDinamik/vox-jot-speech-analysis-runtime`. MLX Sortformer remains user-facing but routes through the separate app-managed `mlx-audio` runtime.
- June 7, 2026 security update: the managed speech-analysis runtime moved to patched `transformers==5.0.0` / `huggingface-hub==1.18.0`. NeMo Sortformer and WhisperX are blocked in the app catalog until they have a compatible non-vulnerable runtime path; do not re-add `nemo_toolkit` or `whisperx` to `speech-analysis-requirements.txt` just to make those historical benchmark rows runnable.
