...
- DiariZen is removed from the user-facing app catalog and Testing view. Granite, Cohere Transcribe, PyAnnote, NeMo Sortformer, Reverb, WhisperX, and Polyvoice stay in the user-facing speech-analysis hub and route through the managed speech-analysis runtime group at `IrieDinamik/vox-jot-speech-analysis-runtime`. MLX Sortformer remains user-facing but routes through the separate app-managed `mlx-audio` runtime.
- June 8/15, 2026 security/runtime update: the managed speech-analysis runtime keeps patched `nemo_toolkit[asr]==2.6.2` after GitHub Dependabot flagged high-severity NeMo advisories patched in 2.6.1/2.6.2, but uses resolver-compatible `transformers==4.53.3` and `huggingface-hub==0.36.0` because NeMo 2.6.2 requires `transformers~=4.53.0`. WhisperX remains pinned at `whisperx==3.4.2`. If future package updates conflict with the patched runtime, fix the runtime by choosing compatible pins or a separate app-managed environment instead of blocking a visible model row.
