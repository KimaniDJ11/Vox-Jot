...
- If a speech-analysis catalog model is downloaded but cannot run, treat the benchmark row as `failed` / needs-fix and repair the bridge/runtime before keeping it visible as runnable. Do not leave a visible speech-analysis row in a blocked final state.
...
- Higgs Audio v3 TTS 4B: keep blocked and out of the user-facing app catalog. The 2026-06-07 validation of `bosonai/higgs-audio-v3-tts-4b` found a public ungated HF repo at commit `e47035915457a842f97199cf7dc005abe5ad93da` with a 9.3 GB BF16 `model.safetensors`, but its Boson Higgs Audio v3 Research and Non-Commercial License does not permit Vox Jot production/end-user distribution without a separate commercial license. The app-managed `mlx-audio` venv contains a `higgs_audio_v3` module, but the official config `model_type: higgs_multimodal_qwen3` currently fails `mlx_audio.tts.utils.load_model` before weights load; a patched-config probe only reached missing-weight handling. Do not add catalog, download, or ranked Testing rows until the app-path loader is fixed. If the model is later exposed for users who already have a Boson commercial license, use `LicenseAcknowledgementGate.kind = "commercial_license_required"`, mark the license manifest `commercial_status` the same way, avoid Vox Jot-hosted mirroring unless distribution rights are granted, and then run full installed-app hard/style/voice-clone suites.
- VoxCPM2: 8-bit outscored both the older 4-bit and bf16 rows. bf16 is slower and less accurate than 8-bit.
- OuteTTS: the Qwen3 `mlx-community/OuteTTS-1.0-0.6B-*` checkpoints still fail in mlx-audio 0.4.x by reaching DAC decode with no audio token arrays (`[concatenate] No arrays provided`). The catalog uses `mlx-community/Llama-OuteTTS-1.0-1B-4bit` instead; keep the Qwen3 0.6B variants out until upstream generation emits decodable `<|c1_*|>/<|c2_*|>` tokens.
...
- MOSS-TTS 8B: removed from the user-facing app catalog after full-suite failure. Do not re-add without a successful app-path run through the pinned mlx-audio runtime.
...
- DiariZen and NeMo Sortformer are removed from the user-facing app catalog and Testing view. Granite, Cohere Transcribe, PyAnnote, Reverb, WhisperX, and Polyvoice stay in the user-facing speech-analysis hub and route through the managed speech-analysis runtime group at `IrieDinamik/vox-jot-speech-analysis-runtime`. MLX Sortformer remains user-facing and routes through the separate app-managed `mlx-audio` runtime.
- July 5, 2026 security/runtime update: GitHub flagged `transformers==4.53.3` in the managed speech-analysis runtime as vulnerable. The runtime moved to the Transformers 5 line, and NeMo Sortformer stayed excluded because the latest tested `nemo_toolkit[asr]==2.7.3` still requires `transformers~=4.57.0`. WhisperX remains pinned at `whisperx==3.4.2`.
- July 18, 2026 security/runtime update: the managed speech-analysis environment now resolves with `torch==2.13.0`, `torchaudio==2.11.0`, `torchvision==0.28.0`, `torchcodec==0.15.0`, `transformers==5.5.0`, `huggingface-hub==1.5.0`, and `onnx==1.22.0`. These are the patched floors for the current GitHub advisories; the runtime marker must change with these pins so existing installations are rebuilt. Keep the `torch.jit.script` guard as defense in depth, and do not re-add NeMo to the visible catalog or shared requirements until it resolves with `transformers>=5.5.0`.
...
- June 7, 2026 MLX-Audio net-new model validation: the app-managed `mlx-audio` venv now pins `mlx-audio[server,stt,tts]==0.4.4` with `phonemizer-fork`, `g2p_en`, `misaki[zh]`, and `setuptools<81`. KittenTTS nano/micro/mini passed one-case bridge synthesis after the Kitten sine/F0 length patch. MisoTTS 8-bit/bf16 passed one-case bridge synthesis after the 0.4.4 runtime restored the Sesame `llama-8B` flavor. Mega-ASR 8-bit completed the full 35-clip Mini LibriSpeech Live STT suite with WER 0.044, 30/35 normalized matches, p50 228 ms, p50 RTF 0.10. Nemotron 3.5 ASR completed the same full live STT suite after the 0.4.4 runtime and inline STT server patch with WER 0.041, 31/35 normalized matches, p50 100 ms, p50 RTF 0.04. File ASR five-format rerun through the app-managed MLX runtime completed for Mega-ASR and Nemotron with WER 0.143 and non-empty transcripts on every sample.
