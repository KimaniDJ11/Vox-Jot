...
- June 8, 2026 security update: the managed speech-analysis runtime moved to patched `transformers==5.0.0` / `huggingface-hub==1.18.0` and `nemo_toolkit[asr]==2.6.2` after GitHub Dependabot flagged high-severity NeMo advisories patched in 2.6.1/2.6.2. WhisperX remains pinned at `whisperx==3.4.2`. If future package updates conflict with the patched runtime, fix the runtime by choosing compatible pins or a separate app-managed environment instead of blocking a visible model row.
