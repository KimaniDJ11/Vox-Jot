...
- June 7, 2026 security update: the managed speech-analysis runtime moved to patched `transformers==5.0.0` / `huggingface-hub==1.18.0`. NeMo Sortformer remains runnable with `nemo_toolkit[asr]==2.1.0`; WhisperX remains runnable with `whisperx==3.4.2`. If future package updates conflict with the patched runtime, fix the runtime by choosing compatible pins or a separate app-managed environment instead of blocking a visible model row.
