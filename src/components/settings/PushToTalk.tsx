  ...
  return (
    ...
    >
      <div className="grid gap-2 sm:grid-cols-2">
        <ModeCard
          title={t("settings.general.pushToTalk.modes.toggle.title")}
          description={t(
            "settings.general.pushToTalk.modes.toggle.description",
          )}
          selected={!selectedPushToTalk}
          disabled={disabled}
          onSelect={() => void updateSetting("push_to_talk", false)}
          firstState={t("settings.general.pushToTalk.states.keyPress")}
          secondState={t("settings.general.pushToTalk.states.nextPress")}
          secondActive={false}
        />
        <ModeCard
          title={t("settings.general.pushToTalk.modes.hold.title")}
          description={t("settings.general.pushToTalk.modes.hold.description")}
          selected={selectedPushToTalk}
          disabled={disabled}
          onSelect={() => void updateSetting("push_to_talk", true)}
          firstState={t("settings.general.pushToTalk.states.holdKey")}
          secondState={t("settings.general.pushToTalk.states.release")}
          secondActive={false}
        />
      </div>
    </SettingContainer>
    ...
    <div className="flex items-center gap-2 rounded-md border border-[var(--border)] bg-[var(--card)] px-2 py-2">
      ...
      {active ? (
        <Mic
          ...
          aria-label={t("settings.general.pushToTalk.states.micOn")}
        />
      ) : (
        <MicOff
          className="h-4 w-4 text-[var(--muted)]"
          aria-label={t("settings.general.pushToTalk.states.micOff")}
        />
      )}
    </div>
  );
